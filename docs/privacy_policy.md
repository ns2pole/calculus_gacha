...
> **公開用の正本**: 本ファイルの内容を Notion 等の公開ページ（`LegalUrls.privacyPolicy`）へ反映してください。  
> 最終更新: 2026-06-03
...
- **AI チューター Plus 等のサブスクリプション状態**（有効 / 無効、有効期限など）  
  例: `users/{Firebase UID}/entitlements/ai_tutor_subsc`
...
- **サブスクリプション状態（無料 / 有料）に応じた利用上限判定情報**  
  無料: 月 10 回、有料（AI チューター Plus 有効時）: 月 500 回（いずれも日本時間の月単位）。当月の利用回数をサーバー側で集計します。

**補足（サーバー上の会話本文）**: Cloud Functions は回答生成の処理中のみ会話内容を扱い、**会話本文をサーバー側のデータベースに永続保存することはありません**。一方、上記のとおり **会話履歴は Cloud Firestore に保存**され、端末内にもキャッシュされます。
...
- **利用上限管理**: `ai_chat_usage` に月次の利用回数等を保持。
...
| AI サブスク状態 | `users/{uid}/entitlements/ai_tutor_subsc` |
