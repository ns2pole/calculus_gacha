...
class FirestoreSettingsService {
  ...
  // ============================================================================
  //
  // NOTE: アプリ本体は因数分解オプションを RevenueCat + ローカル prefs で扱っており、
  // 以下の Firestore ドキュメントは現状どの画面からも呼ばれていない（将来の同期用の残骸）。

  /// 有料オプション購入状態を保存
  static Future<bool> savePremiumPurchased({
    required String userId,
    ...
    try {
      ...
      AppLogger.success('有料オプション購入状態を保存しました', details: 'purchased: $purchased');
      return true;
    } catch (e) {
      final errorStr = e.toString().toLowerCase();
      if (errorStr.contains('permission') || errorStr.contains('permission-denied')) {
        AppLogger.warning('有料オプション購入状態の保存が権限エラーで失敗しました',
          details: 'Firestoreセキュリティルールを確認してください。FIRESTORE_SECURITY_RULES.mdを参照してください。');
        _logAuthStatus(userId, 'savePremiumPurchased', isError: true);
      } else {
        AppLogger.error('有料オプション購入状態の保存に失敗しました', error: e);
      }
      return false;
    }
  }
  ...
  /// 有料オプション購入状態を取得
  static Future<Map<String, dynamic>?> getPremiumPurchased({
    required String userId,
  }) async {
    try {
      ...
      AppLogger.success('有料オプション購入状態を取得しました');
      return {
      ...
      final errorStr = e.toString().toLowerCase();
      if (errorStr.contains('permission') || errorStr.contains('permission-denied')) {
        AppLogger.warning('有料オプション購入状態の取得が権限エラーで失敗しました',
          details: 'Firestoreセキュリティルールを確認してください。FIRESTORE_SECURITY_RULES.mdを参照してください。');
        _logAuthStatus(userId, 'getPremiumPurchased', isError: true);
      } else {
        AppLogger.error('有料オプション購入状態の取得に失敗しました', error: e);
      }
      return null;
    }
  }
  ...
  static Future<Map<String, dynamic>> getAllSettings({
    ...
    try {
      ...
      final results = await Future.wait([
        ...
        getPremiumPurchased(userId: userId),
      ]).timeout(
