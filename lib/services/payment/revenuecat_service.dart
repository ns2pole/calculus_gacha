...
class RevenueCatService {
  ...
  static Future<bool> initialize({String? iosApiKey, String? androidApiKey}) async {
    ...
    try {
      ...
      final providedKey = isIos ? (iosApiKey ?? _defaultIosApiKey) : (androidApiKey ?? _defaultAndroidApiKey);

      if (providedKey.isEmpty || providedKey.startsWith('YOUR_')) {
        _initializationError = 'RevenueCat API key is not configured for ${isIos ? 'iOS' : 'Android'}.';
        // デバッグモードでのみ警告を表示（本番環境では静かに失敗）
        if (kDebugMode) {
          debugPrint('⚠️  [WARNING] RevenueCat: 初期化に失敗しました - $_initializationError');
        }
        return false;
      }
      ...
      try {
        ...
        debugPrint('RevenueCat: Initialized (Customer ID: ${customerInfo.originalAppUserId})');
      } catch (e) {
  ...
  static Future<bool> checkConnection() async {
    ...
    try {
      ...
      debugPrint('RevenueCat: Connection OK (Customer ID: ${info.originalAppUserId})');
      return true;
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> testStoreKitConfiguration(String productId) async {
    if (!kEnableLearningHistoryAndStoreKitDiagnostics) {
      return {
        'success': false,
        'error': 'StoreKit diagnostics disabled (set kEnableLearningHistoryAndStoreKitDiagnostics)',
      };
    }
    if (!Platform.isIOS) {
    ...
    try {
      ...
      final result = await platform.invokeMethod('testProductRequest', {'productId': productId});
  ...
  /// 購入判定ヘルパー
  static bool _hasPurchasedProduct(CustomerInfo customerInfo, String productId, String entitlementId) {
  ...
  /// 因数分解オプションの購入状態
  static Future<bool> isFactorizationOptionPurchased() async {
    if (!_isInitialized) {
      final ok = await initialize();
      if (!ok) return false;
    }
  ...
  static Future<bool> isLearningHistoryOptionPurchased() async {
    ...
    try {
      ...
      return _hasPurchasedProduct(
        info,
        LearningHistoryIapIds.productId,
        LearningHistoryIapIds.entitlementId,
      );
    } catch (e) {
  ...
  static Package? _findPackageForProduct(Offerings offerings, String productId) {
  ...
  /// 因数分解オプション購入
  static Future<PurchaseResult> purchaseFactorizationOption() async {
  ...
  static Future<PurchaseResult> purchaseLearningHistoryOption() async {
    if (!kEnableLearningHistoryAndStoreKitDiagnostics) {
      return PurchaseResult(
        success: false,
        error: 'Learning history IAP disabled (set kEnableLearningHistoryAndStoreKitDiagnostics)',
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (products.isEmpty) {
          ...
          return PurchaseResult(success: false, error: 'Product not found: $productId');
        }
  ...
  static Future<bool> restorePurchases() async {
    ...
    try {
      ...
      // いずれかの entitlement があれば true とする（必要に応じて絞る）
  ...
  static Future<String?> getFactorizationOptionPrice() async {
    ...
    try {
      ...
      debugPrint('RevenueCat: Error getting product price: $e');
      return null;
    }
  }

  /// 価格取得（後方互換性のためのエイリアス）
  /// @deprecated 代わりに getFactorizationOptionPrice() を使用してください
  static Future<String?> getProductPrice() async {
    return getFactorizationOptionPrice();
  }
  ...
  static Future<String?> getLearningHistoryOptionPrice() async {
    ...
    try {
      ...
      try {
        final offerings = await Purchases.getOfferings();
        if (offerings.current != null && offerings.current!.availablePackages.isNotEmpty) {
          final pkg = _findPackageForProduct(offerings, LearningHistoryIapIds.productId);
          if (pkg != null) {
            final price = pkg.storeProduct.priceString;
            debugPrint('RevenueCat: Found learning history price from offerings: $price (productId: ${pkg.storeProduct.identifier})');
            return price;
          }
          // 商品IDが見つからない場合は、最初のパッケージを返さない（nullを返す）
          debugPrint('RevenueCat: Learning history product not found in offerings. Looking for: ${LearningHistoryIapIds.productId}');
      ...
      // offerings 取得できないなら直接 getProducts
      try {
        debugPrint('RevenueCat: Trying getProducts for learning history: ${LearningHistoryIapIds.productId}');
        final products = await Purchases.getProducts([LearningHistoryIapIds.productId]);
        if (products.isNotEmpty) {
          final price = products.first.priceString;
          debugPrint('RevenueCat: Found learning history price from getProducts: $price (productId: ${products.first.identifier})');
          return price;
        } else {
          debugPrint('RevenueCat: No products found for learning history: ${LearningHistoryIapIds.productId}');
        }
      } catch (e) {
        debugPrint('RevenueCat: getProducts for price failed: $e');
      }

      debugPrint('RevenueCat: Could not get learning history price, returning null');
      return null;
    } catch (e) {
      debugPrint('RevenueCat: Error getting learning history product price: $e');
      return null;
    }
  }
