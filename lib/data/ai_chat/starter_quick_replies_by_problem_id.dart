...
//9x^2 - 24xy + 16y^2 : 因数分解ガチャ
  "14E178D7-FC09-4439-B145-92688182FC44": [
    ...
    AiChatQuickReply(label: "最初は何から考えればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "ヒントをください", actionId: "hint"),
...
//4x^2 - 20xy + 25y^2 : 因数分解ガチャ
  "15CEC271-B2C8-40A9-9B84-C64590472E61": [
    ...
    AiChatQuickReply(label: "最初の一歩は何をすればいい？", sendText: "因数分解を始めるにあたって、最初の一歩として何を考えればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "この式はどんな形に見える？", sendText: "この式は、何か特定の因数分解の公式に当てはまりそうな形に見えますか？", actionId: "hint"),
  ],
...
//\int_{(1)/(4)}^{(3)/(4)} \frac{dx}{\sqrt{x(1-x)}} : 積分ガチャ
  "17D1526C-1324-4C82-8D05-59B9308E14D2": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "分母の根号の中の式を簡単にするにはどうすればいいですか？", actionId: "first_step"),
  ],
...
//\int_{0}^{\sqrt{3}} \frac{x}{\sqrt{1+x^{2}}} dx : 積分ガチャ
  "183166B0-C425-4604-B488-2D51C73267D5": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "何から手をつければいい？", actionId: "first_step"),
...
//\int_{-1}^{1}(1-x^{2})^{3} dx : 積分ガチャ
  "18A36D0E-7123-4A0D-8AE6-E8453099D9A9": [
    ...
    AiChatQuickReply(label: "被積分関数は偶関数ですか？奇関数ですか？", actionId: "first_step"),
...
//x^4 + 4 : 因数分解ガチャ
  "1EC3C384-44E0-4C3B-8FE4-2E3814FD7CB3": [
    ...
    AiChatQuickReply(label: "x^4 + 4 の形を見て、何を考えればいい？", sendText: "x^4 + 4 という形を見たときに、最初に何を考えればいいですか？", actionId: "first_step"),
  ],
...
//x^4 + x^2 - 2 : 因数分解ガチャ
  "1F0413C9-47F5-44A8-A3C3-519334677C68": [
    ...
    AiChatQuickReply(label: "因数分解のヒントが欲しいです。", actionId: "hint"),
...
//x^3 - 9xy + 27y^3 + 1 : 因数分解ガチャ
  "24D47C58-831B-4D01-A30E-3BD84A0D741E": [
    ...
    AiChatQuickReply(label: "この式を見て、最初に何を試すべきですか？", actionId: "first_step"),
    AiChatQuickReply(label: "x^3 + 27y^3 の部分から始めますか？", actionId: "hint_specific_part"),
  ],
// : 積分ガチャ
  "26D1B6CC-77F8-4522-88A0-0A06A41D8D67": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "最初の計算ステップを教えてください。", actionId: "first_step"),
...
//x^3 + 125 : 因数分解ガチャ
  "2BC52EFA-C75B-4E77-B1E5-E7ECCA113394": [
    ...
    AiChatQuickReply(label: "125はどんな数ですか？", actionId: "first_step"),
    AiChatQuickReply(label: "ヒントをください", actionId: "hint"),
  ],
//f''(t) = -4 f(t) + 2, f(0) = 0,\ f'(0) = 0 : 微分方程式ガチャ
  "2C1016F1-C7DD-4BE8-8BC8-0C5AA7F05FB5": [
    ...
    AiChatQuickReply(label: "初期条件をどのように使いますか？", actionId: "hint"),
  ],
//\lim_{x\to 0}\frac{1-\cos x}{x^{2}} : 極限ガチャ
  "2D7ED2C9-2759-46FC-B597-619EB2FA6A3B": [
    ...
    AiChatQuickReply(label: "ヒントをください", sendText: "ヒントをください。", actionId: "hint"),
  ],
...
//\lim_{x\to -\infty}\left(1+(1)/(x)\right)^{x} : 極限ガチャ
  "38DF36DC-0268-4E1D-A341-14A39DDE8543": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "x → -∞ の極限はどう考えればいいですか？", actionId: "first_step"),
...
//x^5 + x^4 + x^3 + x^2 + x + 1 : 因数分解ガチャ
  "3A5214AB-C386-4C28-AA01-886809BDE55A": [
    ...
    AiChatQuickReply(label: "最初の一歩は何をすればいいですか？", sendText: "この問題を解くための最初の一歩は何をすればいいですか？", actionId: "first_step"),
...
//x^2 - 7x - 18 : 因数分解ガチャ
  "3CEFDD7D-4880-48DF-ABD8-7779C2D0F2C5": [
    ...
    AiChatQuickReply(label: "最初の一歩は何から始めるべき？", sendText: "この問題を解くための最初の一歩は何から始めるべきですか？", actionId: "first_step"),
  ],
...
//\int_0^{ (\pi)/(2)} \sin^4 x dx : 積分ガチャ
  "422D4451-7399-4187-863C-165C15969BB4": [
    ...
    AiChatQuickReply(label: "sin^4 x の積分はどうすればいいですか？", actionId: "first_step"),
...
//(1)/(x) + (1)/(y) + (1)/(z) = 1 \qquad (0\le x \le y \le z) : 不定方程式ガチャ
  "479E7E6E-ED63-4F1A-BB39-42C4C598782D": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "x, y, z の範囲をどう絞り込みますか？", actionId: "first_step"),
...
//\lim_{n\to\infty}\frac{n!}{n^{n}} : 極限ガチャ
  "48677C5D-732B-4C47-B4AA-645616D8C392": [
    ...
    AiChatQuickReply(label: "分数を積の形に書き換える方法は？", actionId: "first_step"),
  ],
...
//\int_{2}^{5}(x-2)^2(5-x)^2 dx : 積分ガチャ
  "48B05BE5-7632-4228-BF46-F8476E23E0CF": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "解き方のヒントを教えてください", actionId: "hint"),
    AiChatQuickReply(label: "最初の一歩は何をすればいいですか？", actionId: "first_step"),
...
//\lim_{n\to\infty}\sum_{k=1}^{n}(1)/(k) : 極限ガチャ
  "4E839CEA-B995-4D8D-812D-C23546487B0E": [
    ...
    AiChatQuickReply(label: "最初の一歩として何を考えればいいですか？", sendText: "この極限を求めるために、最初の一歩として何を考えれば良いですか？", actionId: "first_step"),
...
//f'(t)=g, f(0) = f_0 : 微分方程式ガチャ
  "5FDE5472-2F31-46C7-89A5-F564360CD6C5": [
    ...
    AiChatQuickReply(label: "f'(t)は何を表していますか？", actionId: "term_meaning"),
    AiChatQuickReply(label: "初期条件f(0)=f_0はどう使いますか？", actionId: "first_step"),
...
//3x^2 + 8xy + 4y^2 - 11x - 6y - 4 : 因数分解ガチャ
  "656CDF93-3968-4527-81AE-9376B3E5B66B": [
    ...
    AiChatQuickReply(label: "まずは何から手をつければいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "因数分解のコツを教えてください", actionId: "hint"),
...
//f''(t)=2, f(0)=-1,\ f'(0)=3 : 微分方程式ガチャ
  "6B6506AA-7296-4C18-9A00-44CAF515FDD4": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "最初は何をすればいいですか？", actionId: "first_step"),
...
//a^3 + 18ab - 8b^3 + 27 : 因数分解ガチャ
  "6C4F2FA4-742F-474E-8103-E3BA3E979350": [
    ...
    AiChatQuickReply(label: "3乗の項が3つあるように見えるけど？", actionId: "hint"),
    AiChatQuickReply(label: "最初の一歩は何をすればいい？", actionId: "first_step"),
  ],
...
//5x^2 + 2xy + y^2 - 12x + 4y + 11 = 0 : 不定方程式ガチャ
  "6ED3659C-A465-4E25-A2B2-4CAF4FEAEDA5": [
    ...
    AiChatQuickReply(label: "xとy、どちらの文字について整理するのが良いですか？", actionId: "first_step"),
  ],
...
//x^4 + x^2 + 1 : 因数分解ガチャ
  "703AE189-789C-4F2D-83A4-55C68DD65991": [
    ...
    AiChatQuickReply(label: "最初の一歩は何から考えればいい？", sendText: "この式を見て、最初の一歩として何から考えればいいでしょうか？", actionId: "first_step"),
    AiChatQuickReply(label: "x^4の項があるときの考え方は？", sendText: "x^4のような高い次数の項がある場合、どのように因数分解を考えれば良いですか？", actionId: "hint"),
  ],
//(1)/(x) + (1)/(y) = (1)/(7) : 不定方程式ガチャ
  "70FFD2EF-5335-4593-86B4-0060A4295D31": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "まず何をすればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "分母に文字がある方程式はどう解きますか？", actionId: "hint"),
  ],
...
//x^2 + 13y^2 = 413 : 不定方程式ガチャ
  "73F50D01-0B10-4D81-A565-163E48987C2E": [
    ...
    AiChatQuickReply(label: "xの範囲を絞るには？", actionId: "first_step"),
    AiChatQuickReply(label: "ヒントをください", actionId: "hint"),
  ],
...
//\lim_{x\to 0}\frac{\sqrt{1+x}-1}{x} : 極限ガチャ
  "774E49FD-A51A-4756-916A-52C94A3810BA": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "分母が0になる形をどう処理すればいいですか？", actionId: "first_step"),
  ],
...
//\lim_{x\to\infty}\int_{0}^{x}t^{5}e^{-t} dt : 極限ガチャ
  "7A1F3B90-6E2C-4D8A-9B4F-1C2D3E4F5A33": [
    ...
    AiChatQuickReply(label: "t^n e^{-t} の積分についてヒントがほしい", actionId: "hint"),
  ],
//64x^3 - 27y^3 : 因数分解ガチャ
...
//49x^2 - 36y^2 : 因数分解ガチャ
  "7D86EF83-52D7-4F6D-9A35-E085E17DA59C": [
    ...
    AiChatQuickReply(label: "この式はどんな形に見える？", actionId: "hint"),
    AiChatQuickReply(label: "最初の一歩はどうすればいい？", actionId: "first_step"),
  ],
...
//a_{n+1} = 6a_n + b_n,\\[4pt] : 漸化式ガチャ
  "863C1DB8-6D69-4926-93D3-24FE77194C4D": [
    ...
    AiChatQuickReply(label: "まずは何をすればいい？", actionId: "first_step"),
    AiChatQuickReply(label: "3項間漸化式に帰着させるには？", actionId: "hint_three_term"),
  ],
//36x^2 - 25y^2 : 因数分解ガチャ
  "897713C5-D885-4103-937C-62F11F4E5C18": [
    AiChatQuickReply(label: "因数分解の基本方針を教えて", actionId: "approach_only"),
    AiChatQuickReply(label: "この式はどんな形に見える？", actionId: "first_step"),
    AiChatQuickReply(label: "36x^2 や 25y^2 をどう変形すればいい？", actionId: "hint"),
  ],
//x^3 + 2x^2 - 5x - 6 : 因数分解ガチャ
  "8B1C27DD-2BAC-41F5-8D5B-2B1F58AB12ED": [
    AiChatQuickReply(label: "因数分解の一般的な方針を教えて", sendText: "この式の因数分解の一般的な方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "最初に何をすればいい？", sendText: "この問題を解くために、最初に何をすればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "因数を見つけるヒントがほしい", sendText: "因数を見つけるためのヒントをください。", actionId: "hint"),
  ],
...
//x^4 - 3x^3 + 3x^2 - 3x + 2 : 因数分解ガチャ
  "8C5A89E9-86F8-4300-9179-3C9CF22FDF90": [
    ...
    AiChatQuickReply(label: "ヒントを教えて", actionId: "hint"),
  ],
...
//\int_{3}^{6}\frac{1}{x^{2}-4} dx : 積分ガチャ
  "8FB8536A-3433-4F7A-8B29-B44DC4186F09": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "分母のx^2-4はどう処理すればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "定積分の計算で注意することはありますか？", actionId: "definite_integral_tip"),
  ],
...
//\sum_{k=1}^{\infty}\frac{1}{2^{k}}\sin\!\left((2k\pi)/(3)\right) : 極限ガチャ
  "9A2E7C44-1B3A-4D7B-9E7F-2E3C1F6B8C10": [
    AiChatQuickReply(label: "まずは何から考えればいいですか？", actionId: "first_step"),
    ...
    AiChatQuickReply(label: "部分和を考えるのはどうですか？", actionId: "hint"),
  ],
//f'(t)=2, f(0)=1 : 微分方程式ガチャ
  "9B75DE01-6B72-487F-B653-EE769057E58B": [
    ...
    AiChatQuickReply(label: "まず何を計算すればいいですか？", actionId: "first_step"),
  ],
//\int_{ \frac {\pi} {2}}^{ \frac {5} {2}\pi}\sqrt{1+\sin x}\ dx : 積分ガチャ
  "9BC1C756-BFC5-41C7-8751-D78DF6430084": [
    ...
    AiChatQuickReply(label: "根号を外すにはどうすればいいですか？", actionId: "first_step"),
...
//\lim_{x\to 0}(\cos x-\cos x^2)/(x-x^2) : 極限ガチャ
  "9D8F2E21-0F37-4C8B-A5B4-7A4F2B3A0C11": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "分母が0に近づくので、どうすればいいですか？", actionId: "first_step"),
  ],
...
//\int_0^1 \sqrt{1 + 4x^2} dx : 積分ガチャ
  "A1B2C3D4-E5F6-7890-ABCD-EF1234567890": [
    ...
    AiChatQuickReply(label: "積分の計算手順の最初の一歩は何ですか？", actionId: "first_step"),
  ],
//3^x + 3^{-x} + 9^x + 9^{-x} = 4 : 三角指数対数ガチャ
  "A1B2C3D4-E5F6-A7B8-C9D0-E1F2A3B4C5D6": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "3^xと9^xの関係は？", actionId: "first_step"),
    AiChatQuickReply(label: "置き換えをするとしたら何がいいですか？", actionId: "hint"),
...
//\int_{(\pi)/(6)}^{(\pi)/(2)} (dx)/(\sin x) : 積分ガチャ
  "A1B2C3D4-E5F6-G7H8-I9J0-K1L2M3N4O5": [
    ...
    AiChatQuickReply(label: "1/sin x の積分はどうすればいい？", actionId: "first_step"),
    AiChatQuickReply(label: "三角関数の分数式の積分でよく使うテクニックは？", actionId: "hint"),
  ],
//\int_{0}^{1}\frac{1}{1+x^{3}} dx : 積分ガチャ
  "A36F411F-99B5-40E0-8BE1-F9126992BA14": [
    AiChatQuickReply(label: "まずは方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "分母の因数分解から教えてください", actionId: "first_step"),
...
//\lim_{n\to\infty}\sqrt[n]{n} : 極限ガチャ
  "A39DF72B-47BA-473A-83D4-5D35E8ADD533": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "n乗根の極限はどう考えますか？", actionId: "first_step"),
...
//(\int_0^t f(s) ds)/(C)=A\sin(\omega t), f(0)=0 : 微分方程式ガチャ
  "A3ECB1EE-67C3-4B41-B8ED-13669F16A63D": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "この式をどう変形すればf(t)が求まる？", actionId: "first_step"),
...
//\int_{-(\pi)/(4)}^{(\pi)/(4)} \cos^3 x dx : 積分ガチャ
  "A5B6C7D8-E9F0-1234-5678-9ABCDEF01234": [
    ...
    AiChatQuickReply(label: "cos^3 x の積分はどうすればいい？", actionId: "first_step"),
...
//a^3 - 4a b^2 - 4b^2 c + a^2 c : 因数分解ガチャ
  "A5BA5B5C-1557-48C1-B4D3-7C57FC5E3A41": [
    AiChatQuickReply(label: "因数分解の基本的な方針を教えて", actionId: "approach_only"),
    AiChatQuickReply(label: "どこから手をつければいい？", actionId: "first_step"),
...
//x^3 + 64 : 因数分解ガチャ
  "A60F2EC9-A23B-420B-89BE-6D8708B0DFA8": [
    AiChatQuickReply(label: "この問題の方針を教えてください", sendText: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "64をどう変形すればいいですか？", actionId: "first_step"),
  ],
...
//x^2 + 6y^2 = 223 : 不定方程式ガチャ
  "A8B9C0D1-E2F3-4A5B-6C7D-8E9F0A1B2C3": [
    ...
    AiChatQuickReply(label: "xとyの範囲をどう絞り込む？", actionId: "first_step"),
...
//\lim_{x\to(\pi)/(2)}\cos(3x)\tan(5x) : 極限ガチャ
  "A8B9C0D1-E2F3-4A5B-6C7D-8E9F0A1B2C3D": [
    ...
    AiChatQuickReply(label: "xがπ/2に近づくとき、cos(3x)とtan(5x)はそれぞれどうなりますか？", actionId: "first_step"),
...
//a_n = \frac {1}{2}(a_{n-1})^2,\qquad a_1 = 5 \qquad (n\ge2) : 漸化式ガチャ
  "AB08A968-36A0-40E9-B392-651BAE535DA9": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "この漸化式のタイプは何ですか？", actionId: "identify_type"),
    AiChatQuickReply(label: "最初の一手として何を試すべきですか？", actionId: "first_step"),
...
//\int_{-2}^{2} x^4 \sin^5 x dx : 積分ガチャ
  "AC0C8F06-65FE-4FDD-8DB8-04252764D3CB": [
    ...
    AiChatQuickReply(label: "被積分関数が偶関数か奇関数か確認するには？", actionId: "first_step"),
...
//\lim_{n\to\infty}\sum_{k=1}^{n}\frac{(-1)^{k+1}}{k} : 極限ガチャ
  "AF028FDC-E4ED-4B5D-907A-FD053AA682F0": [
    ...
    AiChatQuickReply(label: "部分和 S_n をどう扱えばよいですか？", actionId: "first_step"),
...
//\begin{cases} \sin x + \cos y = 1 \\[8pt] \cos x + \sin y = \sqrt{3} \end{cases}, 0 \leq x < 2\pi, 0 \leq y < 2\pi : 三角指数対数ガチャ
  "B2C3D4E5-F6A7-B8C9-D0E1-F2A3B4C5D6E7": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "この連立方程式をどう解き始めればいいですか？", actionId: "first_step"),
...
//x^7 + x^6 + x^5 + x^4 + x^3 + x^2 + x + 1 : 因数分解ガチャ
  "B38E9107-4CB3-4966-AAA8-6966FC3F8B17": [
    AiChatQuickReply(label: "因数分解の一般的な方針を教えて", sendText: "この式の因数分解の一般的な方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "最初の一歩は何をすればいい？", sendText: "この問題を解くための最初の一歩は何をすればいいですか？", actionId: "first_step"),
  ],
...
//x^2 - 2xy - 2x + 6y - 6 = 0 : 不定方程式ガチャ
  "B8F9C5D2-4E6A-4F8B-9C1D-3A5B7C9D1E2F": [
    ...
    AiChatQuickReply(label: "xとyのどちらに注目すべきですか？", actionId: "first_step"),
...
//\int_{-(\pi)/(6)}^{(\pi)/(6)} (dx)/(\cos^3 x) : 積分ガチャ
  "B9B22B73-65CB-4AE7-A6BF-79E52EEDA3BC": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "この定積分の問題、どう考え始めればいいですか？", actionId: "first_step"),
...
//x^3 - 2x^2 - 5x + 6 : 因数分解ガチャ
  "B9C0D1E2-F3A4-5B6C-7D8E-9F0A1B2C3D4": [
    ...
    AiChatQuickReply(label: "最初に何を試せばいい？", sendText: "因数分解を始めるにあたって、最初に何を試せば良いですか？", actionId: "first_step"),
    AiChatQuickReply(label: "因数分解のヒントが欲しい", sendText: "因数分解のヒントをください。", actionId: "hint"),
  ],
...
//\lim_{n\to\infty} n\!\left(\sqrt{n+1}-\sqrt{n}\right) : 極限ガチャ
  "BCC5BF01-27BE-4D8C-8E40-8923C764BBEC": [
    ...
    AiChatQuickReply(label: "最初の一歩は何をすればいい？", sendText: "この問題の最初の一歩は何をすればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "ヒントが欲しい", sendText: "ヒントをください。", actionId: "hint"),
  ],
//\int_{2}^{2\sqrt{3}} \frac{dx}{\sqrt{16 - x^{2}}} : 積分ガチャ
  "BDCAAAB3-8109-4E6B-8A45-9EFD024A0B05": [
    AiChatQuickReply(label: "この積分の解き方の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "分母のルートの中身が気になります。どう変形しますか？", actionId: "first_step"),
  ],
...
//\lim_{n\to\infty}\sum_{k=1}^{n}\frac{(-1)^{k-1}}{2k-1} : 極限ガチャ
  "BE3EC919-60B4-4334-BE8F-389731CF565D": [
    ...
    AiChatQuickReply(label: "問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "最初のステップは何をすべきですか？", actionId: "first_step"),
...
//\int_{-1}^{1}\frac{1}{x^{2}+2x+5} dx : 積分ガチャ
  "C08EC860-F248-4E0A-90FD-F321E66616A4": [
    AiChatQuickReply(label: "分母の式変形について教えて", sendText: "分母の式変形について教えてください。", actionId: "first_step"),
    AiChatQuickReply(label: "定積分の計算方針が知りたい", sendText: "この定積分の計算方針について教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "この形の積分は初めてです", sendText: "この形の積分は初めてなのですが、どう考えれば良いですか？", actionId: "hint"),
  ],
...
//\lim_{x\to 0}x\sin\!\left((1)/(x)\right) : 極限ガチャ
  "C1F3E989-594D-446A-8E94-FD7E70C7450F": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "sin(1/x) の範囲はどう考えればよいですか？", actionId: "first_step"),
...
//2\sin^2 x - \sin x - 1 = 0, 0 \leq x < 2\pi : 三角指数対数ガチャ
  "C286F6A2-9859-42E0-9719-C29CE61F82AC": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "sin x を別の文字に置き換えてもいいですか？", actionId: "first_step"),
...
//\lim_{x\to 0}(1)/(x)\int_{0}^{x}\sqrt{1+\cos^{3}t} dt : 極限ガチャ
  "C2A4D7E9-8C31-4B67-9D1C-7B1F9F4D2C22": [
    ...
    AiChatQuickReply(label: "最初の一歩として何を考えればいいですか？", actionId: "first_step"),
  ],
...
//f''(t)=g,\ f(0)=f_0,\ f'(0)=v_0 : 微分方程式ガチャ
  "C5C2514A-CF06-4F19-BE62-4F99731FA905": [
    ...
    AiChatQuickReply(label: "f'(t) を求めるにはどうすればいい？", actionId: "first_step"),
...
//\lim_{n\to\infty}(1)/(n)\sqrt[n]{{}_{2n}P_n} : 極限ガチャ
  "C9E8A1B2-4D7F-4A91-9F5C-2B6E3A1D8C77": [
    ...
    AiChatQuickReply(label: "まずはどこから手をつければ良いですか？", actionId: "first_step"),
  ],
//x^4 + 5x^2 + 9 : 因数分解ガチャ
  "CAF59AF0-A107-4D21-B0E3-2C3D888A83D2": [
    AiChatQuickReply(label: "因数分解の基本的な方針を教えて", sendText: "この式の因数分解の基本的な方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "x^4 + 5x^2 + 9 をどう変形すればいい？", sendText: "この式を因数分解するために、まずどう変形すれば良いですか？", actionId: "first_step"),
...
//a^2(b-c) + b^2(c-a) + c^2(a-b) : 因数分解ガチャ
  "CE7A2246-F134-4248-86EA-31A0502F58F1": [
    AiChatQuickReply(label: "因数分解の基本方針を教えて", actionId: "approach_only"),
    AiChatQuickReply(label: "どの文字について整理すればいい？", actionId: "first_step"),
    AiChatQuickReply(label: "この式の対称性について教えて", actionId: "hint"),
    AiChatQuickReply(label: "まずは式を展開すればいい？", actionId: "first_step"),
  ],
...
//\int_0^{(\pi)/(2)} \cos^2 x dx : 積分ガチャ
  "D04D1417-91C9-4E4F-8558-80C8FF96A50F": [
    AiChatQuickReply(label: "この積分の解き方の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "cos^2 x の積分はどうすればいいですか？", actionId: "first_step"),
...
//\int_{0}^{3} \frac{dx}{\sqrt{x^2+9}} : 積分ガチャ
  "D36E79F5-FF3C-442A-8A3A-DE453F25EA60": [
    ...
    AiChatQuickReply(label: "分母の√x^2+9をどう処理すればいいですか？", actionId: "first_step"),
...
//\lim_{n\to\infty}(1)/(n)\sum_{k=-n}^{n}\sqrt{1-\left((k)/(n)\right)^{2}} : 極限ガチャ
  "D38E0151-81F6-4A38-A100-93A0C0E7A048": [
    ...
    AiChatQuickReply(label: "定積分の形にどう変換すればいいですか？", actionId: "first_step"),
  ],
//\frac{3^x + 3^{-x}}{2} = 5 : 三角指数対数ガチャ
  "D7300A7C-4D0A-4F2D-B723-48757CD78ED0": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "3^xと3^{-x}の関係について教えてください", actionId: "term_explanation"),
    AiChatQuickReply(label: "指数方程式を解くときのポイントは？", actionId: "hint"),
    AiChatQuickReply(label: "最初の計算は何をすればいいですか？", actionId: "first_step"),
  ],
...
//3x + 5y = 2 : 不定方程式ガチャ
  "DB513B4F-55DC-4AB9-87B7-1BCF5F47DC6A": [
    ...
    AiChatQuickReply(label: "解き方のヒントが欲しい", actionId: "hint"),
    AiChatQuickReply(label: "まず何から考えればいい？", actionId: "first_step"),
...
//\lim_{h\to +0}\left(1+(1)/(h)\right)^{h} : 極限ガチャ
  "E03C5F81-3445-4D78-8BC7-9BDD3EF528CD": [
    ...
    AiChatQuickReply(label: "対数を使うのはなぜですか？", actionId: "why_logarithm"),
    AiChatQuickReply(label: "最初の一歩として何から始めればいいですか？", actionId: "first_step"),
  ],
//x^2 - 9x - 22 : 因数分解ガチャ
  "E0D650DF-7B76-4A37-A0FE-B18A8A041E0B": [
    AiChatQuickReply(label: "因数分解のやり方を教えて", actionId: "approach_only"),
    AiChatQuickReply(label: "最初は何から考えればいい？", actionId: "first_step"),
    AiChatQuickReply(label: "ヒントをください", actionId: "hint"),
  ],
//4x - 7y = 1 : 不定方程式ガチャ
  "E2B55755-0658-4FBE-911F-C18148799924": [
    ...
    AiChatQuickReply(label: "まず何から手をつければいい？", sendText: "この問題はまず何から手をつければいいですか？", actionId: "first_step"),
...
//\int_{0}^{(\pi)/(6)} \sin^3 x dx : 積分ガチャ
  "E356DB2A-1686-4B3F-8F5C-77FBE782E17F": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "sin^3 x の積分はどうすればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "三角関数の3乗の積分は、何か公式がありますか？", actionId: "hint"),
  ],
//x^4 + 4y^4 : 因数分解ガチャ
  "E406BD56-74DE-4603-9695-AE08B0757BE7": [
    AiChatQuickReply(label: "この式の因数分解の基本的な方針は何ですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "何かヒントをください。", actionId: "hint"),
    AiChatQuickReply(label: "最初の一歩として何を考えればいいですか？", actionId: "first_step"),
...
//\lim_{x\to 0}(\tan x)/(x) : 極限ガチャ
  "E5CDA38B-2CD3-4F62-AB20-2F4ECDB14681": [
    AiChatQuickReply(label: "この問題の方針を教えてください", sendText: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "tan x をどう変形すればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "最初の一歩は何ですか？", sendText: "この問題を解くための最初の一歩は何ですか？", actionId: "first_step"),
  ],
//f'(t)+a f(t)=b : 微分方程式ガチャ
...
//xyz = x + y + z \qquad (0< x \le y \le z) : 不定方程式ガチャ
  "E8EA2D7A-4614-47AB-8934-8C7BCF3568C1": [
    AiChatQuickReply(label: "どういう方針で解き進めますか？", actionId: "approach_only"),
    AiChatQuickReply(label: "x, y, z の関係式から何が言えますか？", actionId: "first_step"),
    AiChatQuickReply(label: "不等式 0 < x <= y <= z をどう使いますか？", actionId: "hint"),
    AiChatQuickReply(label: "x, y の候補を絞り込むには？", actionId: "hint"),
  ],
...
//2^{2x+1} - 2^{x+2} = 2^x - 2 : 三角指数対数ガチャ
  "EBEA916F-13BA-4F47-8827-8D2546F80AA0": [
    AiChatQuickReply(label: "どういう方針で解き進めればいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "2^{2x+1} や 2^{x+2} をどう変形しますか？", actionId: "first_step"),
    AiChatQuickReply(label: "指数方程式を解くときのポイントは？", actionId: "hint"),
  ],
...
//xy-yz+zu-ux : 因数分解ガチャ
  "F14C7A82-9E10-4B3D-9C2A-1D8E7F6A5B4C": [
    ...
    AiChatQuickReply(label: "この式をどう整理すればいい？", sendText: "この式を因数分解するために、まずどう整理すればいいですか？", actionId: "first_step"),
...
//(y-z)^3+(z-x)^3+(x-y)^3 : 因数分解ガチャ
  "F1A2B3C4-D5E6-F7A8-B9C0-D1E2F3A4B5C6": [
    ...
    AiChatQuickReply(label: "式を簡単に扱うための工夫はありますか？", actionId: "first_step"),
    AiChatQuickReply(label: "この問題のヒントをください", actionId: "hint"),
  ],
//\int_{0}^{(\pi)/(4)} (dx)/(\cos^2 x) : 積分ガチャ
  "F1G2H3I4-J5K6-L7M8-N9O0-P1Q2R3S4T5U6": [
    AiChatQuickReply(label: "この問題の方針を教えてください。", actionId: "approach_only"),
    AiChatQuickReply(label: "積分の計算で何から手をつければ良いですか？", actionId: "first_step"),
...
//\lim_{n\to\infty}\sqrt{n^{2}+n}-n : 極限ガチャ
  "F7C0451B-E946-4AB6-90C4-A3B539CE4A1D": [
    AiChatQuickReply(label: "この問題の方針を教えてください", actionId: "approach_only"),
    AiChatQuickReply(label: "最初の一歩は何をすればいいですか？", actionId: "first_step"),
    AiChatQuickReply(label: "ルートがあるときの極限の考え方は？", actionId: "hint"),
  ],
//\sin x \cos x + \sqrt{2}(\sin x + \cos x) = (1)/(2), 0 \leq x < 2\pi : 三角指数対数ガチャ
  "F8A3B2C1-9D4E-5F6A-7B8C-9D0E1F2A3B4C": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "sin x + cos x を t と置くのはどうですか？", sendText: "sin x + cos x を t と置くと、式はどうなりますか？", actionId: "first_step"),
    AiChatQuickReply(label: "sin x cos x の部分をどう処理すればいいですか？", actionId: "hint"),
  ],
...
//a(b^2+c^2) + b(c^2+a^2) + c(a^2+b^2) + 2abc : 因数分解ガチャ
  "FAA00A56-4049-4921-A9C8-93B3F420DA1C": [
    ...
    AiChatQuickReply(label: "何から手をつければいいですか？", actionId: "first_step"),
...
//2-2+2-2+2-2+\cdots : 極限ガチャ
  "FAF62CC1-1FE8-41FC-B1B6-5151D088742A": [
    ...
    AiChatQuickReply(label: "最初は何から考えればいい？", sendText: "この問題に取り組む上で、最初に何から考えれば良いですか？", actionId: "first_step"),
  ],
//17x + 29y + 61z = 3 : 不定方程式ガチャ
  "FBFAA113-A701-4D94-9421-3AEF3FA2A96E": [
    AiChatQuickReply(label: "不定方程式の解き方の方針を知りたい", actionId: "approach_only"),
    AiChatQuickReply(label: "3変数の方程式はどう解き始める？", actionId: "first_step"),
...
//\int_0^{(\pi)/(3)} \sin^2 x \cos^2 x dx : 積分ガチャ
  "FEE633FE-DE9A-4BF9-8E42-52FD6CA67E3A": [
    AiChatQuickReply(label: "どういう方針で解けばいいですか？", actionId: "approach_only"),
    AiChatQuickReply(label: "sin^2 x cos^2 x の部分をどう変形しますか？", actionId: "first_step"),
  ],
...
//2^{x+1} - 2^{x-1} = 3 : 三角指数対数ガチャ
  "TRIG-EXP-LOG-002": [
    ...
    AiChatQuickReply(label: "2のx乗でくくるにはどうすればいいですか？", actionId: "first_step"),
