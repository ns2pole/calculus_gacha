...
import 'package:flutter/foundation.dart'; // 追加（ValueNotifier 等）
...
class _ScratchPaperPageState extends State<ScratchPaperPage> with WidgetsBindingObserver {
  ...
  // 学習履歴オプションの購入状態
  bool _isLearningHistoryOptionPurchased = false;
  // 無料で履歴管理可能かどうか（選択された2つのガチャまたはPro版購入済み）
  ...
  String get _scratchDataKey {
    if (widget.problem != null) {
      return 'problem_${widget.problem!.id}';
    }
    return 'general';
  }
  ...
  // 計算用紙データを読み込む
  void _loadScratchData() {
    try {
      final data = SimpleDataManager.getScratchData(_scratchDataKey);
      if (data == null || data.isEmpty) return;

      final List<List<DrawingPoint>> loadedStrokes = [];
      for (final strokeData in data) {
        if (strokeData is List) {
          final List<DrawingPoint> stroke = [];
          for (final pointData in strokeData) {
            if (pointData is Map<String, dynamic>) {
              stroke.add(DrawingPoint(
                Offset(pointData['x'] as double, pointData['y'] as double),
                Color(pointData['color'] as int),
                pointData['width'] as double,
                pointData['isEraser'] as bool? ?? false,
              ));
            }
          }
      ...
      if (mounted && loadedStrokes.isNotEmpty) {
        setState(() {
          _strokes = loadedStrokes;
        });
      }
    } catch (e) {
      debugPrint('Error loading scratch paper data: $e');
    }
  }

  // 計算用紙データをメモリに保存
  void _saveScratchData() {
    try {
      if (_strokes.isEmpty) {
        SimpleDataManager.clearScratchData(_scratchDataKey);
        return;
      }
      ...
      SimpleDataManager.saveScratchData(_scratchDataKey, data);
    } catch (e) {
      debugPrint('Error saving scratch paper data to memory: $e');
    }
  }
          ...
          appBar: AppBar(
            ...
            actions: [
              ...
              Builder(builder: (context) {
                ...
                final l10n = AppLocalizations.of(context)!;
                final localizedHint = widget.problem?.getLocalizedHint(context);
                final hasHint = localizedHint != null && localizedHint.isNotEmpty;
                return Row(
                    ...
                    if (widget.problem != null) ...[
                      ...
                      GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Icon(
                            ...
                            color: _learningStatus == LearningStatus.none ? Colors.white : _learningStatus.color,
                            size: iconSize,
                          ),
                        ),
                      ),
                      ...
                      GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Icon(
                            Icons.save,
                            color: _learningStatus != LearningStatus.none ? Colors.white : Colors.grey[400]!,
                            size: iconSize,
                          ),
                        ),
                      ),
                              ...
                              child: Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (_showHint &&
                                        widget.problem!.getLocalizedHint(context) != null &&
                                        widget.problem!.getLocalizedHint(context)!.isNotEmpty) ...[
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Align(
                                              alignment: Alignment.centerLeft,
                                              child: MixedTextMath(
                                                widget.problem!.getLocalizedHint(context)!,
                                                forceTex: true,
                                                labelStyle: const TextStyle(fontSize: 18),
                                  ...
                                  ...widget.problem!.getLocalizedSteps(context).map((s) {
                                    return Padding(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ListTile(
              title: Text(l10n.drawWithFinger),
              trailing: Switch(
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    if (_currentTool == DrawingTool.partialEraser || _currentTool == DrawingTool.strokeEraser || _isEraser) {
      setState(() {
  ...
  void _onPointerMove(PointerMoveEvent event) {
    ...
    if (_currentTool == DrawingTool.strokeEraser || _currentTool == DrawingTool.partialEraser || _isEraser) {
  ...
  void _onPointerCancel(PointerCancelEvent event) {
    ...
    if (_currentTool == DrawingTool.strokeEraser || _currentTool == DrawingTool.partialEraser || _isEraser) {
      _saveScratchData();
    } else if (!_isEraser && _points.isNotEmpty) {
      setState(() {
        _strokes.add(List.from(_points));
        _points = [];
        _redoStack.clear();
  ...
  Future<void> _saveImage() async {
    ...
    try {
      ...
      if (!status.isGranted) {
        _showSnackBar(l10n.storagePermissionRequired);
        return;
      }
      ...
      final file = File('${directory.path}/scratch_paper_$timestamp.png');
      await file.writeAsBytes(pngBytes);

      _showSnackBar(l10n.imageSaved(file.path));
    } catch (e) {
      _showSnackBar(l10n.saveFailed(e.toString()));
    }
  }
    ...
    try {
      ...
      switch (_learningStatus) {
        ...
        case LearningStatus.understood:
          problemStatus = ProblemStatus.understood;
          break;
