...
List<ProblemLevel> kGachaLevelOrder = const [
  ProblemLevel.easy,
  ProblemLevel.mid,
...
class _GachaSettingsPageState extends State<GachaSettingsPage> {
  ...
  Widget _buildSlotDifficultyRow(int idx) {
    final l10n = AppLocalizations.of(context)!;
    final levelLabels = [l10n.easy, l10n.mid, l10n.advanced];
    
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(l10n.difficultyForProblem(idx + 1), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ...
        Wrap(
          ...
          children: [
            for (var j = 0; j < levelLabels.length; j++) ...[
              GestureDetector(
                onTap: () async {
                  setState(() {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(levelLabels[j], style: const TextStyle(fontSize: 16)),
                  ],
                ),
              ),
            ],
      ...
      case GachaFilterMode.excludeSolvedGE1:
        return SingleChildScrollView(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${l10n.latestN(1)} ', style: TextStyle(fontSize: 14, color: Colors.grey[900])),
              _statusBadgeSmall(LearningStatus.solved, diameter: 18.0),
              const SizedBox(width: 2),
              Text(l10n.removeFromGacha, style: TextStyle(fontSize: 14, color: Colors.grey[900])),
            ],
          ),
        );
      case GachaFilterMode.excludeSolvedGE2:
        return SingleChildScrollView(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${l10n.latestN(2)} ', style: TextStyle(fontSize: 14, color: Colors.grey[900])),
              _statusBadgeSmall(LearningStatus.solved, diameter: 18.0),
              const SizedBox(width: 2),
              Text(l10n.removeFromGacha, style: TextStyle(fontSize: 14, color: Colors.grey[900])),
            ],
      ...
      case GachaFilterMode.excludeSolvedGE3:
        return SingleChildScrollView(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${l10n.latestN(3)} ', style: TextStyle(fontSize: 14, color: Colors.grey[900])),
              _statusBadgeSmall(LearningStatus.solved, diameter: 18.0),
              const SizedBox(width: 2),
              Text(l10n.removeFromGacha, style: TextStyle(fontSize: 14, color: Colors.grey[900])),
            ],
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    // 微分方程式ガチャの場合はlevel選択を非表示
                    if (widget.prefsPrefix != 'physics_math') ...[
                      const Divider(),
                      const SizedBox(height: 8),
                      Text(l10n.selectDifficultyForEachSlot, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                      const SizedBox(height: 8),
                    ...
                    Text(l10n.exclusionRuleDescription, style: const TextStyle(fontSize: 16)),
                    // 微分方程式ガチャの場合はカテゴリー選択の説明を追加
                    if (widget.prefsPrefix == 'physics_math') ...[
                      const SizedBox(height: 12),
                      Text(l10n.pmGachaKeywordDescription, style: const TextStyle(fontSize: 16, fontStyle: FontStyle.italic)),
                      Text(l10n.keywordSelectionInGachaScreen, style: const TextStyle(fontSize: 16, fontStyle: FontStyle.italic)),
                    ],
