#!/usr/bin/env python3
"""
全問題について、問題と答えのみのペアを箇条書きで記したファイルを生成するスクリプト
"""
import re
import os
import sys
...
def extract_problems_from_dart_file(file_path):
    ...
    for i, line in enumerate(lines):
        ...
        # 括弧の深さを追跡（), は2つの閉じ括弧として扱う）
        # ), や ); を先に処理してから括弧をカウント
        line_for_counting = line
        if '),' in line:
            # ), を ) ) に置き換えてカウント（実際には2つの閉じ括弧として扱う）
        ...
        if 'MathProblem(' in line and not stripped.startswith('//'):
            ...
            # 問題ブロックの終了を検出
            # brace_depthが0になった時点で、その行が),または);で終わっているかを確認
            is_end = False
            if brace_depth <= 0:
...
def main():
    ...
    # ガチャごとのファイル定義（all_problems.dartに基づく）
    gacha_files = {
        '積分ガチャ': [
    ...
    # 出力ファイルに書き込み
    output_file = 'problems_and_answers_list.txt'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("全問題の「問題と答え」リスト（ガチャごと）\n")
        f.write("=" * 80 + "\n\n")
