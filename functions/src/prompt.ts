...
export function buildSystemInstruction(request: AiChatRequest): string {
  ...
  const instructions = locale === "en"
    ? buildEnglishInstructions(wantsBriefGuidance)
    : buildJapaneseInstructions(wantsBriefGuidance);

  const contextBlock = locale === "en"
    ? buildEnglishContext(context)
    : buildJapaneseContext(context);

  return [...instructions, "", ...contextBlock].join("\n");
}

function buildJapaneseInstructions(wantsBriefGuidance: boolean): string[] {
  return [
...
function buildEnglishInstructions(wantsBriefGuidance: boolean): string[] {
  return [
...
export function retryTooLongMessage(locale: AiChatLocale): string {
  if (locale === "en") {
    return "Your previous response was too long and got cut off. " +
      "Please shorten it significantly—if it's a hint or approach, keep it to 2–3 sentences. " +
      "Make sure all sentences and math expressions are complete.";
  }
  return "直前の返答は長すぎて途中で切れています。" +
...
export function retryBrokenLatexMessage(locale: AiChatLocale): string {
  if (locale === "en") {
    return "Your previous response may have broken LaTeX syntax. " +
      "Please verify matching $...$, $$...$$, {}, and \\left/\\right pairs, " +
      "break complex expressions into shorter pieces, and rewrite with the same content in a renderable form.";
  }
  return "直前の返答はLaTeX構文が壊れている可能性があります。" +
    "$...$、$$...$$、{}、\\left/\\right の対応を確認し、" +
